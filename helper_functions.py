...
def calc_voxel_size(lambda_photon, d_detector, s_pixel, dim):
    """Calculate voxel size in nm^-1 with the crystallographers' convention assuming cubic array."""
    pixel_num = dim - dim//2
    min_angle = np.arctan(s_pixel / d_detector)
    max_angle = np.arctan(np.sqrt((pixel_num * s_pixel)**2 + (pixel_num * s_pixel)**2) / d_detector)
    
    ewald_rad = d_detector / s_pixel
    q_max = (2. * np.sin(0.5 * max_angle)) / lambda_photon
    q_sep = ((2. * np.sin(0.5 * min_angle)) / lambda_photon) * (d_detector / ewald_rad / s_pixel)
    fov = 1 / q_sep
    
    voxel_size = fov / dim
    half_p_res = 0.5 / q_max
    max_p_res = 2 * half_p_res
    return voxel_size, half_p_res, max_p_res
